...
# 2026-09-11 a shell started without it logged the user out): the six session
# variables are removed, --sm-disable is passed, GSETTINGS_BACKEND=memory keeps
# dconf writes local. On top of that, XDG_DATA_HOME and XDG_CONFIG_HOME point
# at a scratch directory, so the headless shell loads the extension from THIS
# checkout (copied there by this script) and never reads the user's installed
# copy or rules.json.
#
...
SCRATCH="${HEADLESS_SCRATCH:-${TMPDIR:-/tmp}/wctl-headless-$STAMP}"
...
echo "Stop it with: kill ${SHELL_PID:-$RUNNER_PID}   (never pkill -- it matches the real shell)" >&2
